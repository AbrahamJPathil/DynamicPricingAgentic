...
# from kafka_publisher import publish_proposal, flush as kafka_flush  # Kafka disabled - no broker on this machine
...
# kafka_publisher.py - they write to a local JSONL file acting as a mock
# "inventory-agent" topic instead of producing to Kafka. Nothing else in this
# file needs to change. To restore real Kafka, delete this block and
# uncomment the import above.
_ROOT = Path(__file__).resolve().parent.parent.parent
_MOCK_KAFKA_DIR = os.getenv("MOCK_KAFKA_DIR", str(_ROOT / "data" / "mock_kafka"))
os.makedirs(_MOCK_KAFKA_DIR, exist_ok=True)
_MOCK_TOPIC_PATH = os.path.join(_MOCK_KAFKA_DIR, "inventory-agent.jsonl")
...
def _load_recent_history(sku: str, limit: int = 3) -> List[dict]:
    ...
    dependency on Kafka/mock-topics either way, so it keeps working unchanged
    whichever transport build_output_node is using.
